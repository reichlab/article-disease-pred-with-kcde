...
for(data_set in all_data_sets) {
    ...
    num_rows <- length(all_prediction_horizons) *
        ...
        length(all_prediction_statistics)
    
    data_set_results <- data.frame(data_set = data_set,
        model = "SARIMA",
        prediction_horizon = rep(NA_integer_, num_rows),
        prediction_time = rep(NA, num_rows),
        prediction_statistic_name = rep(NA_character_, num_rows),
        prediction_statistic_value = rep(NA_real_, num_rows),
    ...
    for(prediction_horizon in all_prediction_horizons) {
        for(prediction_time_ind in prediction_time_inds) {
            ...
            ## Compute each prediction statistic
            for(prediction_statistic_name in all_prediction_statistics) {
                ...
                data_set_results$prediction_statistic_name[results_row_ind] <-
                    prediction_statistic_name
                
                ## Compute the given statistic
                if(identical(prediction_statistic_name, "log_score")) {
                    data_set_results$prediction_statistic_value[results_row_ind] <-
                        dlnorm(observed_prediction_target,
                            meanlog = predictive_log_mean,
                            sdlog = predictive_log_sd,
                            log = TRUE)
                } else if(identical(prediction_statistic_name, "pt_pred")) {
                    data_set_results$prediction_statistic_value[results_row_ind] <-
                    ...
                    if(identical(data_set, "dengue_sj")) {
                        data_set_results$prediction_statistic_value[results_row_ind] <-
                            data_set_results$prediction_statistic_value[results_row_ind] - 1L
                    }
                } else if(identical(prediction_statistic_name, "AE")) {
                    ## Point prediction was previously computed
                    pt_pred_ind <- which(data_set_results$prediction_horizon == prediction_horizon &
                        data_set_results$prediction_time == data$time[prediction_time_ind] &
                        data_set_results$prediction_statistic_name == "pt_pred")
                    
                    pt_pred <- data_set_results$prediction_statistic_value[
                        pt_pred_ind
                    ]
                    
                    data_set_results$prediction_statistic_value[results_row_ind] <-
                        abs(pt_pred - observed_prediction_target)
                } else if(identical(prediction_statistic_name, "interval_pred_lb_95")) {
                    data_set_results$prediction_statistic_value[results_row_ind] <-
                        qlnorm(0.05,
                            meanlog = predictive_log_mean,
                            sdlog = predictive_log_sd)
                ...
                } else if(identical(prediction_statistic_name, "interval_pred_ub_95")) {
                    data_set_results$prediction_statistic_value[results_row_ind] <-
                        qlnorm(0.95,
                            meanlog = predictive_log_mean,
                            sdlog = predictive_log_sd)
                ...
                } else if(identical(prediction_statistic_name, "interval_pred_lb_50")) {
                    data_set_results$prediction_statistic_value[results_row_ind] <-
                        qlnorm(0.25,
                            meanlog = predictive_log_mean,
                            sdlog = predictive_log_sd)
                ...
                } else if(identical(prediction_statistic_name, "interval_pred_ub_50")) {
                    data_set_results$prediction_statistic_value[results_row_ind] <-
                        qlnorm(0.75,
                            meanlog = predictive_log_mean,
                            sdlog = predictive_log_sd)

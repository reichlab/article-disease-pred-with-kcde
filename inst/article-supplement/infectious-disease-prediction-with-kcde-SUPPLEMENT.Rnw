...
conducted by \citet{duong2005crossvalidationBandwidthMultivariateKDE}
demonstrating that a fully parameterized bandwidth matrix could yield improved density estimates for joint density estimation with continuous distributions.
...
capture seasonality.  \citet{Held2012HHHSeasonality} advocate using performance
of one-step-ahead predictions (as measured by the log score, ranked probability
...
Following the examples in \citet{Held2012HHHSeasonality}, we selected the model
with the best mean log score for predictions for the final two years of the
...
individual weeks in the applications. 
...
peak week.  Figure~\ref{fig:CombinedPeakWeekTimingPredictionLogScores} in this
...
take at a single value: the eventual realized outcome.  Figures
...
Figure~\ref{fig:SimTrajectories} illustrates this with the
...
\begin{figure}
<<SimStudyDistributionsDiscretizedDuongHazelton, echo = FALSE, fig.height = 6, dev = "cairo_ps">>=
library(ggplot2)
library(grid)
library(plyr)
library(dplyr)
...
pushViewport(viewport(layout = grid.layout(nrow = 9, ncol = 2,
            heights = unit(c(rep(1, 8), 0.75), c("lines", rep("null", 8))),
            widths = unit(c(1, 0.25), c("null", "null")))))
...
#            facet_wrap(~ data_set_and_season, ncol = 1) +
        ...
        vp = viewport(layout.pos.row = 1 + i, layout.pos.col = 2)
    )
}
...
pushViewport(viewport(layout = grid.layout(nrow = 9, ncol = 2,
            heights = unit(c(rep(1, 8), 0.75), c("lines", rep("null", 8))),
            widths = unit(c(1, 0.25), c("null", "null")))))
grid.text("Summary of Results for Peak Week Timing",
    gp = gpar(fontsize = 14),
    vp = viewport(layout.pos.row = 1, layout.pos.col = 1:2))
...
the method did better than using equal bin probabilities.}
\label{fig:CombinedPeakWeekTimingPredictionLogScores}
\end{figure}

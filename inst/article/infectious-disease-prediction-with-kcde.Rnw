...
ili_prediction_results_sarima <- readRDS("/media/evan/data/Reich/infectious-disease-prediction-with-kcde/results/ili_national/prediction-results/sarima-predictions.rds")
ili_prediction_results_kcde <- readRDS("/media/evan/data/Reich/infectious-disease-prediction-with-kcde/results/ili_national/prediction-results/kcde-predictions.rds")
...
full_model_descriptor <- ili_prediction_results$model
@

<<FluDataRibbonsPredictionPlot, echo = FALSE>>=
...
ggplot() +
    geom_ribbon(aes(x = prediction_time, ymin = lower, ymax = upper, colour = model, fill = model, alpha = interval_type),
        size = 0,
    ...
    geom_line(aes(x = prediction_time, y = pt_pred, colour = model),
        data = ili_prediction_results[ili_prediction_results$prediction_horizon == 1, ]) +
...
ggplot() +
    geom_ribbon(aes(x = prediction_time, ymin = lower, ymax = upper, colour = model, fill = model, alpha = interval_type),
        size = 0,
        data = ribbons_df[ribbons_df$prediction_horizon %in% phs_used & ribbons_df$interval_type == "50", ]) +
    geom_line(aes(x = time, y = weighted_ili), data = ili_national[ili_national$year %in% 2010:2014, ]) +
    geom_point(aes(x = time, y = weighted_ili), data = ili_national[ili_national$year %in% 2010:2014, ]) +
    geom_line(aes(x = prediction_time, y = pt_pred, colour = model),
        data = ili_prediction_results[ili_prediction_results$prediction_horizon == 1, ]) +
...
ggplot() +
    geom_ribbon(aes(x = prediction_time, ymin = lower, ymax = upper, colour = model, fill = model), alpha = 0.4,
        size = 0,
        data = ribbons_df[ribbons_df$prediction_horizon %in% phs_used & ribbons_df$interval_type == "95", ]) +
    geom_line(aes(x = time, y = weighted_ili), data = ili_national[ili_national$year %in% 2010:2014, ]) +
    geom_point(aes(x = time, y = weighted_ili), data = ili_national[ili_national$year %in% 2010:2014, ]) +
    geom_line(aes(x = prediction_time, y = pt_pred, colour = model),
        data = ili_prediction_results[ili_prediction_results$prediction_horizon == 1, ]) +
...
<<FluDataAEPlot, echo = FALSE>>=
mae_sarima <- mean(ili_prediction_results$AE[which(ili_prediction_results$prediction_horizon == 1 & ili_prediction_results$model == "SARIMA")])
mae_kcde <- mean(ili_prediction_results$AE[which(ili_prediction_results$prediction_horizon == 1 & ili_prediction_results$model == "kcde")])

#ae_diff_df <- 
@

<<FluDataLogScorePlot, echo = FALSE>>=
inds_sarima <- which(ili_prediction_results$prediction_horizon == 1 & ili_prediction_results$model == "SARIMA")
inds_sarima_order <- order(ili_prediction_results$prediction_time[inds_sarima])
inds_sarima <- inds_sarima[inds_sarima_order]

inds_kcde <- which(ili_prediction_results$prediction_horizon == 1 & ili_prediction_results$model == "kcde")
inds_kcde_order <- order(ili_prediction_results$prediction_time[inds_kcde])
inds_kcde <- inds_kcde[inds_kcde_order]

log_score_diff <- ili_prediction_results$log_score[inds_kcde] - ili_prediction_results$log_score[inds_sarima]

plot(ili_prediction_results$prediction_time[inds_kcde], log_score_diff)

subinds_52 <- which(week(ili_prediction_results$prediction_time[inds_kcde]) == 52)
points((ili_prediction_results$prediction_time[inds_kcde])[subinds_52], log_score_diff[subinds_52], pch = 18, col = "red")

subinds_53 <- which(week(ili_prediction_results$prediction_time[inds_kcde]) == 53)
points((ili_prediction_results$prediction_time[inds_kcde])[subinds_53], log_score_diff[subinds_53], pch = 18, col = "blue")

subinds_1 <- which(week(ili_prediction_results$prediction_time[inds_kcde]) == 1)
points((ili_prediction_results$prediction_time[inds_kcde])[subinds_1], log_score_diff[subinds_1], pch = 18, col = "green")

inds_below_neg4 <- which(log_score_diff < -4)
(ili_prediction_results$prediction_time[inds_kcde])[inds_below_neg4]

ili_national$log_score_diff <- NA
ili_national$log_score_diff[ili_national$year %in% 2011:2014] <- log_score_diff
ggplot() +
    geom_line(aes(x = time, y = weighted_ili), data = ili_national[ili_national$year %in% 2010:2014, ]) +
    geom_point(aes(x = time, y = weighted_ili, alpha = log_score_diff), colour = "black", data = ili_national[ili_national$year %in% 2010:2014, ]) +
    geom_line(aes(x = prediction_time, y = pt_pred, colour = model),
        data = ili_prediction_results[ili_prediction_results$prediction_horizon == 1, ]) +
    geom_point(aes(x = prediction_time, y = pt_pred, colour = model),
        data = ili_prediction_results[ili_prediction_results$prediction_horizon == 1, ]) +
    #    facet_wrap( ~ ph) +
    theme_bw()
@

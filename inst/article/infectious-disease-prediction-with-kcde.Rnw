...
integrated moving average (SARIMA) models among others.  Brown \etal [in
preparation] and \citet{unkel2012statisticalInfectiousDiseasePredictionReview}
are recent reviews of work on forecasting infectious disease, and describe these alternative
...
copula implemented in the {\tt R} (\citet{RCoreLanguage}) package {\tt copula}
...
\citet{Held2012HHHSeasonality} discuss the fact that the seasonal structure in
...
The estimation methods were implemented in {\tt R} and

...
We model incidence in each remaining week of the season directly; predictions for the second and third prediction targets are derived from a model for the joint distribution of incidence in each remaining week of the season.  Using data available up through time $t^*$, we employ KCDE

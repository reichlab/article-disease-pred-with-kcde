...
\toprule
         & Predictive & Baseline &     \multicolumn{6}{c}{Summary of Log Score Differences} \\
\cline{4-9}
 Disease & Model      & Model    & Min & Q1 & Median & Q3 & Max & Mean (95\% CI) \\ 
  \hline
...
for(row_num in 1:8) {
  cat(paste0("Dengue & ",
    ...
    round(temp[row_num, "min"], 2), " & ",
    round(temp[row_num, "Q1"], 2), " & ",
    round(temp[row_num, "median"], 2), " & ",
    round(temp[row_num, "Q3"], 2), " & ",
    round(temp[row_num, "max"], 2), " & ",
    round(temp[row_num, "mean"], 2), " (",
    round(temp[row_num, "mean_ci_lb"], 2), ", ",
    round(temp[row_num, "mean_ci_ub"], 2), ")",
    "\\\\\n"
  ))
}
cat("\\midrule\n");
...
score differences for predictions of incidence made at all combinations of 
target week in the test period and prediction horizon. THE CONFIDENCE INTERVALS ARE TOO NARROW/NOT CLEAR IF THEY'RE MEANINGFUL.}
\end{table}
...
\cline{4-9}
 Disease & Model      & Model    & Min & Q1 & Median & Q3 & Max & Mean (95\% CI) \\ 
  \hline
<<PerformanceSummaryTableNadir, echo = FALSE, results = "asis">>=
#difference ~ N(mean_difference_model_pair, rho^(target time difference) * phi^(analysis time difference))???
...
temp <- ili_prediction_log_score_diffs_from_sarima_with_pred_time_covars %>%
...
\end{tabular}
\caption{Summaries of model performance relative to the baseline models for
predictions of incidence in individual weeks where observed incidence in the target week was less than one third of the maximum weekly incidence in the test period.  Each row summarizes the log
score differences for predictions of incidence made at all combinations of 
target week in the test period and prediction horizon. THE CONFIDENCE INTERVALS ARE TOO NARROW/NOT CLEAR IF THEY'RE MEANINGFUL.}
\end{table}
...
\cline{4-9}
 Disease & Model      & Model    & Min & Q1 & Median & Q3 & Max & Mean (95\% CI) \\ 
  \hline
<<PerformanceSummaryTablePeak, echo = FALSE, results = "asis">>=
#difference ~ N(mean_difference_model_pair, rho^(target time difference) * phi^(analysis time difference))???
...
\end{tabular}
\caption{Summaries of model performance relative to the baseline models for
predictions of incidence in individual weeks where observed incidence in the target week was at least two thirds of the maximum weekly incidence in the test period.  Each row summarizes the log
score differences for predictions of incidence made at all combinations of 
target week in the test period and prediction horizon. THE CONFIDENCE INTERVALS ARE TOO NARROW/NOT CLEAR IF THEY'RE MEANINGFUL.}
\end{table}

...
structures known as copulas.  We focus our evaluation of prediction methods using targets chosen by the CDC as 
relevant to public health.  The CDC has identified three features of outbreaks that can be used
by public health decision makers and we use these as our prediction targets.
...
To simplify our modeling efforts we focus on the first target and generate predictions
for the second and third as derived quantities.  Using data available up through time $t^*$, we employ KCDE

...
improve discrete conditional density estimates.  In applications to predicting
dengue fever and influenza, our method yields improved predictions for dengue incidence in individual weeks
...
incidence known as HHH4.  KCDE also offers more consistent
performance than the baseline models for predictions of
incidence in the peak week, and is comparable to the baseline models on the
other prediction targets.  The periodic kernel function leads to improved
...
Recent efforts by government officials and academic researchers
have identified features of outbreaks that can be used
by public health decision makers.  In this article we focus
...
trajectory of incidence over the following weeks.  Predictive
...
can also be formulated in the Bayesian framework.  For example,
\citet{zhou2015DirichletProcessCopulaAmphibianDiseaseArrival} model the time to
...
An additional challenge with both agent-based and compartmental models is that
...
model(s) for each dataset using log scores.
...
and Supplemental Figure 4).  The periodic kernel also generally improved
predictions of incidence in the peak week, but it did not have consistent
benefits in predicting peak week timing.
...
outlying cases where the KCDE model assigned up to about 450 times as much
probability to the realized outcome as SARIMA, and over 100 times as much probability as HHH4. 
...
in the seasons with high peak incidence (Figure ***).
...
probability to each incidence bin (Figures *** and ***).  Similarly, SARIMA underperformed
...
the equal bin probabilities model on average (Figures *** and ***).
...
offered considerable improvements over the naive approach.
...
seasons.  KCDE and HHH4 rarely did much worse than using equal bin
probabilities.
...
@
\caption{Log scores for predictions of peak week incidence by predictive
model.  The vertical line is placed at the peak week for
each season.  The log score for ``Equal Bin Probabilities'' is obtained by
assigning equal probability that the peak incidence will be in each of the
specified incidence bins.  There are 11 incidence bins for dengue
and 27 bins for influenza.}

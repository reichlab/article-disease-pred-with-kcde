...
(\citet{duong2005crossvalidationBandwidthMultivariateKDE}), we introduce an
...
Summarizing across all combinations of prediction horizon and prediction target
week, in both data sets the median performance of KCDE specifications without
...
better than the baseline models (Figure~\ref{fig:AggregatedFluResultsBoxPlots}
and Supplemental Figure 4).  
Using the fully parameterized bandwidth matrix generally had little impact on
the quality of the predictive distributions as measured by the log score.  In
all cases, the distribution of log score differences between the KCDE
...
kernel component performed about as well as SARIMA throughout the season. 
In the application to dengue, the KCDE specifications performed similarly to the
baseline models near the season nadir, but 
...
the maximum weekly case count in the testing period), the average log score difference between the predictions
from the Periodic, Full Bandwidth KCDE model and SARIMA was about
<<KCDESARIMALogScoreDiffsdLowerTertile, echo = FALSE, results = "asis">>=
cat(paste0(sprintf("%.3f", round(kcde_sarima_lower_tertile_mean, 3)), " (sd = ", sprintf("%.3f", round(kcde_sarima_lower_tertile_sd, 3)), "),"))
@
...
cat(paste0(sprintf("%.3f", round(kcde_hhh4_lower_tertile_mean, 3)), " (sd = ", sprintf("%.3f", round(kcde_hhh4_lower_tertile_sd, 3)), ")."))
@
% -0.009 (sd = 0.378).
These differences indicate that this KCDE
specification had similar performance to the baseline models in periods of low
incidence.
However, in weeks with more than 184 reported cases (the upper tertile) 
the mean difference in log scores was about
<<KCDESARIMALogScoreDiffUpperTertile, echo = FALSE, results = "asis">>=
cat(paste0(sprintf("%.3f", round(kcde_sarima_upper_tertile_mean, 3)), " (sd = ", sprintf("%.3f", round(kcde_sarima_upper_tertile_sd, 3)), ")"))
@
%1.589 (sd = 1.598)
relative to SARIMA, and about
<<KCDEHHH4LogScoreDiffUpperTertile, echo = FALSE, results = "asis">>=
cat(paste0(sprintf("%.3f", round(kcde_hhh4_upper_tertile_mean, 3)), " (sd = ", sprintf("%.3f", round(kcde_hhh4_upper_tertile_sd, 3)), ")"))
@
%2.060 (sd = 1.324)
relative to HHH4.
...
as SARIMA on average and almost 8 times higher probability as HHH4 on average.  Moreover, there were
...
Similar qualitative patterns also hold with the other KCDE specifications.
...
#        data = dengue_prediction_log_score_diffs_from_sarima_long_with_pred_time_covars) +
#    facet_wrap( ~ model, ncol = 1) +
#    theme_bw()
#
#ggplot() +
#    geom_point(aes(x = total_cases, y = log_score_difference),
#        data = dengue_prediction_log_score_diffs_from_sarima_long_with_pred_time_covars[dengue_prediction_log_score_diffs_from_sarima_long_with_pred_time_covars$model == "KCDE-seasonal_lag_0-differencing_FALSE-periodic_TRUE-bw_full", ]) +
#    facet_wrap( ~ season, ncol = 1) +
#    theme_bw()
...
%    geom_point(aes(x = total_cases, y = log_score_difference),
%        data = dengue_prediction_log_score_diffs_from_hhh4_long_with_pred_time_covars[dengue_prediction_log_score_diffs_from_hhh4_long_with_pred_time_covars$model == "KCDE-seasonal_lag_0-differencing_FALSE-periodic_TRUE-bw_full", ]) +
%    geom_hline(yintercept = 0) +
%    geom_smooth(aes(x = total_cases, y = log_score_difference),
%        method = lm,
%        colour = "#56B4E9",

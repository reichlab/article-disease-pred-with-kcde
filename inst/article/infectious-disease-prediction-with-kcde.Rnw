...
similar covariates.  Using such local methods is a natural idea in predicting
...
the time of the season.  Throughout the manuscript we use the term
covariates to refer to both.
...
to predict incidence after $t^*$.  



When we apply the method to
...
state level predictions we did not use HHH4 as a reference

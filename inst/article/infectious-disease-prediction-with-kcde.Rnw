...
\author{Evan L. Ray$^{1}$, Krzysztof Sakrejda$^1$, Stephen A. Lauer$^1$,\\
Michael A. Johansson$^2$, Nicholas G. Reich$^1$\vspace{0.2cm} \\ 
$^1${\small\textit{Department of
...
$^2${\small\textit{Dengue Branch, Division of Vector-Borne Infectious
Diseases,}}\\
...
\begin{abstract}
{
Creating statistical models that generate accurate predicted trajectories of 
infectious disease incidence over multiple time points is a challenging
methodological problem whose solution could benefit public health decision makers.
We develop a new approach for prediction of infectious disease incidence using
kernel conditional density estimation (KCDE) and copulas.  We use KCDE to obtain
predictive distributions for incidence in individual weeks and use copulas to tie those
distributions together into joint distributions. This strategy enables us to create
...
variables. We demonstrate via simulation that using a fully parameterized bandwidth matrix can
improve conditional density estimates.  In applications to predicting dengue
fever and influenza, our method yields improved predictions for dengue incidence in individual weeks
relative to a seasonal autoregressive integrated moving average (SARIMA) model
and a previously published generalized linear model for infectious disease
incidence known as HHH4.  KCDE also outperforms HHH4 for predictions of
dengue incidence in the peak week, and is comparable to the baseline models on
the other prediction targets.  The periodic kernel function leads to improved
...
Predictions of infectious disease incidence from rigorously evaluated models provide valuable information to
public health officials planning disease prevention and control measures(\citet{Manheim:2016ur}).
...
(\citet{hatchett2007interventionsIntensity1918flu}).  Accurate 
predictions can help target such interventions more effectively.

Recent collaborative efforts by government officials and academic researchers
have identified features of outbreaks that can be used and 
interpreted by public health decision makers.  In this article we focus
on three of these features: weekly incidence, the timing of the season peak, and
incidence in the peak week.  These quantities have emerged as being targets of
particular utility in making planning decisions 
(\citet{PandemicPredictionandForecastingScienceandTechnologyInteragencyWorkingGroup2015Announcement,
EpidemicPredictionInitiative2015Index}).

In this work, we use a non-parametric approach to conditional density
estimation referred to as kernel conditional density estimation (KCDE) and a
parametric method for modeling joint dependence structures known as copulas.
Using data available up through a given time point, we employ KCDE
to obtain separate predictive distributions for
disease incidence in each subsequent week of the season.  We then
combine those marginal distributions using copulas to obtain joint predictive distributions for the
trajectory of incidence over the course of the following weeks.  Predictive
distributions relating to the timing of and incidence at the peak week can be
obtained from this joint predictive distribution for the trajectory of disease
incidence.
...
multivariate densities or regression functions involving discrete variables have
employed a kernel function that is a product of univariate kernel functions
(\citet{aitchison1976multivariateBinaryKernel,
...
% Using a product kernel simplifies the formulation of the kernel
% function when discrete variables are present, but 
This approach forces the
kernel function to be oriented in line with the coordinate axes.  
% In settings with only continuous variables, asymptotic
...
% in many cases (\citet{duong2005crossvalidationBandwidthMultivariateKDE}).  
Motivated by results showing that multivariate
kernel functions with a bandwidth parameterization allowing for
flexible orientations can result in improved density estimates,
we introduce an approach that allows for flexible orientation of discrete kernels 
by discretizing an underlying continuous kernel function
(\citet{duong2005crossvalidationBandwidthMultivariateKDE}).

Copulas present an attractive strategy
for estimating the joint distribution of moderate to high dimensional random
vectors.  Using a simple dependence model, a copula
...
scale well with the dimension of the vector whose distribution is being estimated (\citet{HastieTibshiraniESL}).  This
...
Methods combining non-parametric
estimates of marginal densities with copulas have been considered for other
applications such as economic time series
(\citet{patton2012reviewCopulaEconomicTimeSeries}).

In a time-series context, KCDE is a local method in the sense that the conditional density
estimate for future observations given conditioning variables is a weighted
...
relationship between conditioning and outcome variables. Applications range from
...
measles (\citet{sugihara1990nonlinearForecasting}) and
influenza (\citet{viboud2003predictionInfluenzaMoA}) to sports analytics
where a version of nearest neighbors regression predicts the career trajectories
of current NBA players (\citet{SilverCARMELOPrediction}).
...
KCDE ({e.g.}, \citet{HastieTibshiraniESL} discuss the connection between
nearest neighbors and kernel methods for regression).

% Additionally, KCDE is a simple representation of models proposed for chaotic and non-linear systems in disease, physics, and ecology.
% In our implementation of KCDE, the conditioning variables are recent lags of incidence. In dynamical systems, the use of recent observations of a dynamical process state can be motivated by state space reconstruction, which tells us that recent lags of one variable in a complex system can give us information about trajectory of the system even when other components of the underlying state are unobserved. 
...
time of lung cancer patients (\citet{hall2004crossvalidationKCDE}), female
labor force participation (\citet{hall2004crossvalidationKCDE}), bond yields
and value at risk in financial markets (\citet{fan2004crossvalidationKCDE}),
and wind power (\citet{jeon2012KCDEWindPower}), among others.  Similar methods
can also be formulated in the Bayesian framework.  One example along these lines
is \citet{zhou2015DirichletProcessCopulaAmphibianDiseaseArrival}, who model the
time to arrival of a disease in amphibian populations using Dirichlet processes and copulas.
...
integrated moving average (SARIMA) models among others.  Brown \textit{and
others} (in preparation) and
\citet{unkel2012statisticalInfectiousDiseasePredictionReview} are recent reviews
of work on forecasting infectious disease, and describe these alternative
approaches in more detail.

Little research has been done comparing the predictive performance of more detailed and 
disease-mechanistic modeling approaches (agent-based or compartmental models) to 
more statistical approaches (regression or SARIMA models). 
One challenge in making comparisons to mechanistically detailed models is that 
these models are often highly parameterized and difficult to independently reproduce or replicate.
One of the most well-developed modern statistical frameworks for infectious 
disease prediction is the ``HHH4'' model (\citet{Held2005HHHIntro,
...
Another commonly used and widely studied approach is the seasonal autoregressive
integrated moving average (SARIMA) model. However, both of these approaches 
...
observed (\citet{unkel2012statisticalInfectiousDiseasePredictionReview}).
...
% approaches in this category is the HHH4 model (\citet{Held2005HHHIntro,
% Paul2008MultivariateHHH4, Held2012HHHSeasonality}), a specific variation of a
% generalized linear model developed for infectious disease.
...
There are several key features that distinguish our proposed approach
from existing methods. 
First, we focus on generating full predictive distributions to fully  
...
(Brown \textit{and others}, in preparation), KCDE makes minimal assumptions about the
...
\begin{enumerate}
  ...
  \item Binned incidence in the peak week of the current season:
    ...
    &\qquad = \int_{\{(\bz_{({t^*} + 1):({t^*} + H_{t^*})}): a \leq \max_{w} z_{S_{t^*} + w} < b\}} f(\bz_{({t^*} + 1):({t^*} + H_{t^*})} | {t^*}, \bz_{1:{t^*}}) \, d \bz_{({t^*} + 1):({t^*} + H_{t^*})}. \label{eqn:PeakPredIncidenceIntegral}
    \end{align}
\end{enumerate}
...
incidence at a particular horizon $h$ after time $t^*$.  In order to simplify the notation we define two new variables: $Y_t^{h} = Z_{t + h}$ represents
...
literature on Gaussian Processes (\citet{mackay1998introductionGP}), and is
defined by
\begin{equation}
...
%\begin{align}
%&\hat{f}_{\bY|\bX}(\by | \bX = \bx) = \frac{\hat{f}_{\bY, \bX}(\by, \bx)}{\hat{f}_{\bX}(\bx)} \label{eqn:KDECondDef} \\
...
%obtain
%\begin{align}
%\hat{f}_{\bY|\bX}(\by | \bx) &= \sum_{t \in \btau} w_t K^{\bY | \bX}\left\{\by, \by_t | \bx, \bx_t; \bH^{\bX,\bY}\right\}, \text{ where} \label{eqn:KCDEDefinitionWeighted} \\
...
each component of that vector and $\b1$ is a column vector of ones.
...
distribution (\citet{gneiting2007strictlyProperScoringRules}).
However, its use as an optimization criterion has been criticised for being
sensitive to outliers (\citet{gneiting2007strictlyProperScoringRules}).  In
...
(\citet{schuster1981nonconsistencyLCVforKDE,
...
We use copulas (\citet{nelsen2007introductionCopulas}) to tie the marginal
...
from the marginal distributions.  Figure 10 in the
...
most convenient to frame the discussion in this Section in terms of
...
naturally yield corresponding predictive CDFs
$F^{h}(y_{t^*}^h | \bx_{t^*}; \btheta^h)$.
...
(\citet{HofertRCopulaPackage}).  The copula function is given by
\begin{equation}
...
(\citet{joe2005asymptoticEfficiencyTwoStageCopula}).
...
estimating discrete conditional distributions with KCDE.  There are many factors that
determine the relative performance of KCDE estimators with different bandwidth
parameterizations.  In this simulation study, we vary just one of these factors:
the sample size ($N = 100$ or $N = 1000$).  
% We hold other factors that may be
% related to the relative performance of different bandwidth specifications fixed.
...
the discretized bivariate normal random variable $\bX$.  Using these
...
integrated squared error (\citet{kanazawa1993hellingerDistanceKDE}). For each
...
sets (Figure~\ref{fig:IntialDataPlots}). The first provides a weekly integer number of reported cases of dengue fever in San
Juan, Puerto Rico. The second provides a weekly weighted proportion of
doctor visits with influenza-like illness in the United States.  
These data sets were used in two recent
prediction competitions sponsored by the United States federal government
(\citet{PandemicPredictionandForecastingScienceandTechnologyInteragencyWorkingGroup2015Announcement,
EpidemicPredictionInitiative2015Index}).  
% In the dengue data set, the incidence
% measure is an integer number of reported cases in the given week.  In the
% influenza data set the incidence measure is continuous, a weighted proportion of
% doctor visits with influenza-like illness.
...
reporting.  Specifically, we use only data up through a given week in order to
make predictions for incidence after that week.

We use three prediction targets for each data set, based closely on the
prediction targets that were used in the original competitions (Supplemental Figure 3).  
First, for each
week in the test data, we obtain a predictive distribution for the incidence
measure in that week at each prediction horizon from 1 to 52 weeks ahead. 
Second, in each week of the test data set, we make predictions for the timing of
the peak week of the corresponding season.
...
package (\citet{hyndmanRForecastPackage}); this function uses a stepwise
...
A limitation of SARIMA models is that they are only appropriate for
continuous data.  In applying this model to the dengue data, we have
...
(\citet{Held2005HHHIntro, Paul2008MultivariateHHH4, Held2012HHHSeasonality}),
available in the {\tt surveillance} (\citet{Hohle:R:surveillance}) package in R.
...
We followed the model selection and estimation procedures outlined in \citet{Held2012HHHSeasonality}
(see Supplement for details).  Because
the HHH4 model specifies a Poisson or Negative Binomial distribution for
incidence, it cannot be used with continuous data; therefore, we have only applied this
model with the dengue data.
...
%We performed first-order seasonal differencing to obtain a roughly stationary
%time series and then obtained the final model fits using the {\tt auto.arima}
%function in {\tt R}'s {\tt forecast} package
%(\citet{hyndmanRForecastPackage}); this function uses a stepwise procedure to
%determine the terms to include in the model.
%This procedure resulted in a
%<<ILISarimaModelFitSummary, echo = FALSE, results = "asis">>=
%ili_sarima_fit <- readRDS("/media/evan/data/Reich/infectious-disease-prediction-with-kcde/inst/results/ili_national/estimation-results/sarima-fit.rds")
%temp <- capture.output(summary(ili_sarima_fit)) 
...
%cat(paste0("SARIMA(", substr(temp[2], 7, 15), as.integer(substr(temp[2], 16, 16)) + 1, substr(temp[2], 17, 18), ")$_{52}$"))
%@
%model for the dengue data.  Further details of the SARIMA model specification
%and estimation procedure are in the supplement.

% We begin with a discussion of predictive distributions for incidence
% in individual weeks.  
% Figure~\ref{fig:DengueRibbonsPredictions} displays
% the median and 95\% interval limits for the predictive distributions obtained 
% at prediction horizons of $1$, $6$, or $26$ weeks from SARIMA and from the
% Periodic, Full Bandwidth KCDE model.  Figure 4 in the supplement shows the
% corresponding predictive intervals obtained from HHH4.  
Evaluating the predictions of weekly incidence showed similar performance 
across the methods in many scenarios, although KCDE showed more stable and 
calibrated performance across long horizons, especially in years that had 
unusually high incidence.
For predictions of
dengue fever incidence, at a prediction horizon of one week the point
predictions from all three methods are similar, although the intervals from
...
For the more regularly seasonal influenza data, there is less of a
noticeable distinction between the predictions given by the KCDE and SARIMA
models.
...
% Figure~\ref{fig:AggregatedFluResultsBoxPlots} summarizes
% the results for all of the KCDE specifications in both applications, aggregated
% across all prediction target weeks and prediction horizons.
We evaluated the performance of all KCDE specifications against the baseline 
model(s) for each dataset.
In all cases, the median performance of KCDE specifications without the periodic
kernel is slightly lower than the baseline models, but the median performance of
specifications with the periodic kernel is similar to or slightly better than
the baseline models (Figure~\ref{fig:AggregatedFluResultsBoxPlots} and Supplemental Figure 4).  
Using the fully parameterized bandwidth matrix generally had little
impact on the quality of the predictive distributions as measured by the log score.
% In the application to predicting dengue, 
% % the pattern we
% % saw in Figure~\ref{fig:DengueLogScoreDiffVsReportedCasesCombined} holds for all
% % of the KCDE specifications: 
% all KCDE specifications outperformed the baseline models for
% predictions of high incidence near the season peaks.  
% In Figure~\ref{fig:AggregatedFluResultsBoxPlots}, these cases are visible as
% positive outliers in the boxplots.  This pattern does not hold in the
% application to influenza.  
For the influenza application, the KCDE specifications
including the periodic kernel component performed about as well as SARIMA
throughout the season. 
We chose the Periodic, Full Bandwidth KCDE specification as the best-performing 
KCDE specification for further comparison.
...
In weeks with fewer than 93 reported cases (roughly one third of the maximum weekly case count
in the testing period), the average log score difference between the predictions
from KCDE and SARIMA was about 0.252 (sd = 0.733), where values greater than zero 
show KCDE making more accurate predictions than SARIMA.
The average log score difference in comparison to HHH4 in this incidence range 
was about -0.009 (sd = 0.378). These differences indicate that KCDE had similar
performance to the baseline models in periods of low incidence.
However, in weeks with more than 184 reported cases (the upper tertile) 
the mean difference in log scores was about 1.589 (sd = 1.598) relative to SARIMA, 
and about 2.060 (sd = 1.324) relative to HHH4.
Translating to a probability scale, in these periods of high incidence KCDE
assigned about 5 times higher probability to the observed outcome as SARIMA on average and
almost 8 times higher probability as HHH4 on average.  Moreover, there were
outlying cases where KCDE assigned up to about 450 times as much probability to
the realized outcome as SARIMA, and over 100 times as much probability as HHH4.
...
%
%
%lower_cutoff <- max(dengue_prediction_log_score_diffs_from_hhh4_long_test$total_cases) / 3
%upper_cutoff <- 2 * lower_cutoff
%
%temp <- dengue_prediction_log_score_diffs_from_hhh4_long_test[
%    dengue_prediction_log_score_diffs_from_hhh4_long_test$model == "KCDE-seasonal_lag_0-differencing_FALSE-periodic_TRUE-bw_full" &
%        dengue_prediction_log_score_diffs_from_hhh4_long_test$total_cases <= lower_cutoff,
%    "log_score_difference"]
%
%mean(temp)
%sd(temp)
...
%temp <- dengue_prediction_log_score_diffs_from_hhh4_long_test[
%    dengue_prediction_log_score_diffs_from_hhh4_long_test$model == "KCDE-seasonal_lag_0-differencing_FALSE-periodic_TRUE-bw_full" &
%        dengue_prediction_log_score_diffs_from_hhh4_long_test$total_cases >= upper_cutoff,
%    "log_score_difference"]
%
%mean(temp)
%sd(temp)
...
In all cases and for both diseases, all methods evaluated tend to converge 
rapidly on the truth once the peak week has passed 
(Figure~\ref{fig:CombinedPeakWeekIncidencePredictionLogScores} and 
Supplemental Figures 5 through 9).
...
makers: periods of high incidence near the season peak.  

Overall, across both
data sets our method offered more consistency than either of the
baseline models in early-season predictions for the timing of
and incidence in the peak week. In the application to influenza,
our method did about as well as SARIMA when predicting incidence in individual
weeks.  Our method also outperformed HHH4 for predictions of
incidence in the peak week in the application to dengue.  In seasons with high incidence both baseline 
models made worse predictions than a na\"ive model assigning equal probability to each 
incidence bin, whereas KCDE never did much worse than this na\"ive model.  
These improvements in early-season predictions are valuable to public health 
officials planning interventions several weeks or months before the peak
of the disease season. 
However, since year-to-year variation is substantial, continued evaluation of 
these methods on datasets with longer prospective testing phases could provide better 
information about long-run performance of all of these methods.
...
the dynamics of dengue incidence accurately.  Relaxing that structure by using a
non-parametric approach such as KCDE may yield improved capability to represent
...
and incidence in the peak week in the dengue data.  Similarly, the standard
...
%with the baseline models, and offered the advantage of more stability in the
%performance of predictions made early in the season.
...
grid search suggested by \citet{de2000nonparametricConditionalPredictiveRegions}
would be far too slow for our methods.

Another possibility for addressing this problem would be to replace
variable selection with shrinkage.  \citet{hall2004crossvalidationKCDE}
...
irrelevant conditioning variables tend to infinity asymptotically as the sample
size increases.
...
a dramatically larger sample size.  
%In particular, we hypothesize that a
%penalty on the inverse of the bandwidth matrix encouraging it to have small
%eigenvalues could be helpful.  
If successful, this would also enable further
exploration of using other predictive variables such as weather or 
incidence measures from neighboring locations in the model.
...
variations on integrated squared error (e.g.,
\citet{fan2004crossvalidationKCDE}) or the continuous ranked probability
score (\citet{jeon2012KCDEWindPower}).
...
specification.  Other approaches such as compartmental models require more
tailoring to the specific disease being modeled.  However, in general we would
expect a well-specified parametric model to outperform KCDE.  On the other hand, 
...
well as other methods such as mechanistic models might perform better than any
...
The estimation methods were implemented in {\tt R} (\citet{RCoreLanguage}) and
{\tt C}.  All source code and data are available in {\tt R} packages hosted on GitHub
(\citet{ReichLabGitHubDiseasePredWithKCDEPackage}).
...
#            })) +
    ...
    theme_bw()
@
\caption{Results from the simulation study.  Positive values indicate simulation trials
where the full bandwidth specification outperformed the diagonal bandwidth
specification with the same training data set, as measured by Hellinger distance from the
target conditional density.}
\label{fig:SimStudyResultsPlot}
...
color_palette <- c("#E69F00", "#D55E00", "#56B4E9", "#0072B2")
...
models_used <- c(#"HHH4-seasonal_lag_NA-differencing_NA-periodic_NA-bw_NA",
...
#sarima_contrast_means <- combined_prediction_log_score_diffs_from_sarima_long[combined_prediction_log_score_diffs_from_sarima_long$model %in% models_used, ] %>%
#    group_by(reduced_model_descriptor, data_set) %>%
#    summarize(mean_log_score_difference = mean(log_score_difference))
#hhh4_contrast_means <- combined_prediction_log_score_diffs_from_hhh4_long[combined_prediction_log_score_diffs_from_hhh4_long$model %in% models_used, ] %>%
#    group_by(reduced_model_descriptor, data_set) %>%
#    summarize(mean_log_score_difference = mean(log_score_difference))
...
#    stat_summary(fun.y = "mean", fun.ymin = "mean", fun.ymax = "mean", geom = "point", shape = "-") +
#    geom_point(aes(y = mean_log_score_difference, x = reduced_model_descriptor, colour = data_set),
#        shape = "-",
#        size = 10,
#        position = position_dodge(width = 0.75),
#        data = sarima_contrast_means) +
...
#        data = ili_prediction_log_score_diffs_from_hhh4_long[ili_prediction_log_score_diffs_from_hhh4_long$model %in% models_used, ]) +
#    ylim(c(-4, 4)) +
    scale_colour_manual("Data Set", values = color_palette) +
    scale_x_discrete(labels = c("Periodic, Full Bandwidth KCDE" = "Periodic, Full Bandwidth\nKCDE")) +
...
print(boxplot_hhh4_contrasts, vp = viewport(layout.pos.row = 2, layout.pos.col = 1))
...
%\begin{figure}
%<<DengueLogScoreDiffVsReportedCases, echo = FALSE, fig.height = 8.4, dev = "postscript">>=
%temp <- dengue_sj[, c("time", "total_cases", "season")]
%colnames(temp) <- c("prediction_time", "total_cases", "season")
%
%dengue_prediction_log_score_diffs_from_sarima_long_test <-
...
%
%
%scatterplot_dengue_diff_from_sarima_by_obs_cases <- ggplot() +
%    geom_point(aes(x = total_cases, y = log_score_difference),
%        data = dengue_prediction_log_score_diffs_from_sarima_long_test[dengue_prediction_log_score_diffs_from_sarima_long_test$model == "KCDE-seasonal_lag_0-differencing_FALSE-periodic_TRUE-bw_full", ]) +
%    geom_hline(yintercept = 0) +
...
%    xlab("Reported Dengue Cases in Prediction Target Week") +
%    ylab("Log Score Difference") +
%    ggtitle("(a) Comparison of Periodic, Full Bandwidth KCDE Model and SARIMA\nvs. Reported Dengue Cases") +
%    theme_bw(base_size = 11)
%
%scatterplot_dengue_diff_from_hhh4_by_obs_cases <- ggplot() +
%    geom_point(aes(x = total_cases, y = log_score_difference),
%        data = dengue_prediction_log_score_diffs_from_hhh4_long_test[dengue_prediction_log_score_diffs_from_hhh4_long_test$model == "KCDE-seasonal_lag_0-differencing_FALSE-periodic_TRUE-bw_full", ]) +
%    geom_hline(yintercept = 0) +
%    geom_smooth(aes(x = total_cases, y = log_score_difference),
%        method = lm,
%        colour = "#56B4E9",
...
%    xlab("Reported Dengue Cases in Prediction Target Week") +
%    ylab("Log Score Difference") +
%    ggtitle("(b) Comparison of Periodic, Full Bandwidth KCDE Model and HHH4\nvs. Reported Dengue Cases") +
%    theme_bw(base_size = 11)
%
%grid.newpage()
%pushViewport(viewport(layout = grid.layout(nrow = 2, ncol = 1,
...
%            heights = unit(c(1, 1), c("null", "null")),
%            widths = unit(c(1), c("null")))))
%print(scatterplot_dengue_diff_from_sarima_by_obs_cases, vp = viewport(layout.pos.row = 1, layout.pos.col = 1))
%print(scatterplot_dengue_diff_from_hhh4_by_obs_cases, vp = viewport(layout.pos.row = 2, layout.pos.col = 1))
%@
%\caption{Differences in log scores for the weekly predictive distributions
%obtained from the Periodic, Full Bandwidth KCDE model and the baseline
%models, plotted against the observed incidence in the week being predicted.
...
%prediction horizon.
%}
%\label{fig:DengueLogScoreDiffVsReportedCases}
%\end{figure}
...
#    geom_point(aes(x = analysis_time_season_week, y = peak_week_log_score, colour = reduced_model_descriptor, shape = reduced_model_descriptor)) +
    ...
    scale_linetype_manual("Model",
        ...
        values = c("SARIMA" = 1, "HHH4" = 2, "Null KCDE" = 3, "Full Bandwidth KCDE" = 4, "Periodic KCDE" = 5, "Periodic, Full Bandwidth KCDE" = 6,
            "Equal Bin Probabilities" = 1)) +
#    scale_linetype_manual("Model", breaks = reduced_models_used, values = c(1:6, 1)) +
#    scale_shape_manual("Model", breaks = reduced_models_used, values = c(0:4, 45)) +

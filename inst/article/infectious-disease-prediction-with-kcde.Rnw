...
made as though in real time assuming that once cases are reported, they are
never revised.  Specifically, we use only data up through a given week in
order to make predictions for incidence after that week.
...
the log-transformed incidence measure as a baseline to compare our methods to.  We obtained a 
...
estimation procedure are in the supplemental materials.
...
supplement. Using the fully parameterized bandwidth matrix generally
had little impact on the quality of the predictive distributions as measured by
the log score.
...
scatterplot_dengue_by_obs_cases <- ggplot() +
    ...
    xlab("Observed Total Dengue Fever Cases in Prediction Target Week") +
    ylab("Log Score Difference") +
    ggtitle("(b) Log Score Difference from SARIMA vs. Observed Dengue Fever Cases\nPeriodic, Full Bandwidth KCDE Model") +
    theme_bw(base_size = 11)
...
the supplementary materials.  This ``Christmas effect'' sometimes coincides with

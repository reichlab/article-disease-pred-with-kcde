...
We evaluated model performance for predictions in each data set using log scores
...
\subsubsection{For predictions of incidence in individual weeks, KCDE outperforms the baseline models.}  KDCE specifications including a periodic kernel component consistently had the highest or close to the highest mean log scores for both data sets whether aggregating across all weeks or only high incidence weeks (Table).  Additionally, the worst-case performance of the HHH4 and SARIMA models was much lower than the worst-case performance of any of the KCDE specifications (Table) for all combinations of the data set and the subset of weeks considered.
...
We have seen that the SARIMA model generally had higher mean performance than the KCDE models in the application to influenza (although it also had less consistent performance for predictions of incidence in individual weeks or at the peak week).
...
\toprule
         &       & Target & Actual   \\
 Disease & Model & Coverage & Coverage \\ 
  \hline
...
for(row_num_group_ind in seq_along(row_num_groups_list)) {
  ...
  if(row_num_group_ind %in% c(1, 3)) {
    cat("\\cline{2-4}\n")
  } else if(row_num_group_ind == 2) {
    cat("\\cline{1-4}\n")
  }
}
@
...
the ``Low Incidence" group summarizes log scores for predictions of incidence in weeks where the
observed incidence was less than one third of the maximum weekly incidence in the test period;
...
\begin{tabular}{lllllllll}
\toprule
         &                    &          &              \multicolumn{6}{c}{Summary of Log Score Differences from} \\
         &                    &          &              \multicolumn{6}{c}{Equal Bin Probabilities} \\
\cline{4-9}
 Disease & Subset & Model & Min & Q1 & Q2 & Mean & Q3 & Max \\ 
  \hline
...
temp <- dengue_peak_incidence_log_score_diffs_from_equal_long %>%
  group_by(reduced_model_descriptor) %>%
  summarize(
...
temp_before_peak <- dengue_peak_incidence_log_score_diffs_from_equal_long %>%
...
temp <- ili_peak_incidence_log_score_diffs_from_equal_long %>%
  group_by(reduced_model_descriptor) %>%
  summarize(
    min = min(log_score_difference),
    Q1 = quantile(log_score_difference, probs = 0.25),
...
temp_before_peak <- ili_peak_incidence_log_score_diffs_from_equal_long %>%
  filter(before_peak_week) %>%
  group_by(reduced_model_descriptor) %>%
  summarize(
    min = min(log_score_difference),
...
\flushleft
\label{tbl:IndWeeksIncidenceResults}
\caption{Summaries of model performance relative to a naive approach assigning equal probability to all bins for
predictions of incidence in the peak week.  Each row summarizes the differences in log
score for the given model and the log score that would be achieved by using equal bin probabilities; positive values indicate that the model performed better than using equal bin probabilities.  The ``All Weeks'' group summarizes results for all combinations of 
...
temp <- dengue_peak_timing_log_score_diffs_from_equal_long %>%
  group_by(reduced_model_descriptor) %>%
  summarize(
    min = min(log_score_difference, na.rm = TRUE),
    Q1 = quantile(log_score_difference, probs = 0.25, na.rm = TRUE),
    median = quantile(log_score_difference, probs = 0.5, na.rm = TRUE),
    Q3 = quantile(log_score_difference, probs = 0.75, na.rm = TRUE),
    max = max(log_score_difference, na.rm = TRUE),
    mean = mean(log_score_difference, na.rm = TRUE)
  ) %>%
  ungroup() %>%
  mutate(reduced_model_descriptor = as.character(reduced_model_descriptor),
    week_type = "All Weeks") %>%
...
temp_before_peak <- dengue_peak_timing_log_score_diffs_from_equal_long %>%
  filter(before_peak_week) %>%
  group_by(reduced_model_descriptor) %>%
  summarize(
    min = min(log_score_difference, na.rm = TRUE),
...
temp <- ili_peak_timing_log_score_diffs_from_equal_long %>%
  group_by(reduced_model_descriptor) %>%
  summarize(
    min = min(log_score_difference),
    Q1 = quantile(log_score_difference, probs = 0.25),
...
temp_before_peak <- ili_peak_timing_log_score_diffs_from_equal_long %>%
  filter(before_peak_week) %>%
  group_by(reduced_model_descriptor) %>%
  summarize(
    min = min(log_score_difference),
...
\caption{Summaries of model performance relative to a naive approach assigning equal probability to all bins for
predictions of the peak week timing.  Each row summarizes the differences in log
score for the given model and the log score that would be achieved by using equal bin probabilities; positive values indicate that the model performed better than using equal bin probabilities.  The ``All Weeks'' group summarizes results for all combinations of 
target week in the test period and prediction horizon; the ``Before Peak" group summarizes results
for predictions in weeks before the actual peak for the given season.  The model in \textbf{bold} font
had the highest mean log score within each combination of disease and weeks subset.  The model in \underline{\emph{italicized and underlined}} font had the lowest minimum log score within each combination of disease and weeks subset.  In some cases, the same model had both the highest average log score and the lowest worst-case log score.}

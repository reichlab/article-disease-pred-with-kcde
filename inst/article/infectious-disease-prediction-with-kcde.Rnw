...
infectious disease incidence over multiple time points is a challenging methodological problem whose solution could benefit public health decision makers.
We develop a new approach for prediction of infectious disease incidence using
kernel conditional density estimation (KCDE).  We use KCDE to obtain predictive
distributions for incidence in individual weeks and use copulas to tie those
...
via simulation that allowing for a fully parameterized bandwidth matrix can
improve conditional density estimates.  In applications to predicting dengue
fever and influenza, our method outperforms a seasonal
...
(\citealp{hatchett2007interventionsIntensity1918flu}). Therefore, accurate 
predictions can help target resource allocation more effectively.

Recent collaborative efforts by government officials and academic researchers have focsed on identifying features of outbreaks that can be used and interpreted by public health decision makers.
In this article we focus
on three features: weekly incidence, the timing of the season peak, and incidence
in the peak week. 
These quantities have emerged as being targets of particular utility in making
planning decisions 
...
conditional density estimation (KCDE). Using data available before a given timepoint, we use KCDE to obtain separate predictive
distributions for disease incidence in each subsequent week of the season.  
We then
...
Compared with point predictions, the full predictive distributions generated by KCDE characterize uncertainty in the predictions. 
...
Additionally, unlike many methods common in the infectious disease literature (Brown et al., in preparation), KCDE makes minimal assumptions about the underlying system governing disease dynamics. This flexibility makes KCDE suitable for application to a wide variety of time series, including diseases with different latent dynamics.
...
nonlinear systems, because it imposes little structure on the assumed relationship between conditioning and outcome variables. Applications range from similar infectious disease settings where
...
influenza (\citealp{viboud2003predictionInfluenzaMoA}) to sports analytics where a version of nearest neighbors regression predicts the career trajectories of current NBA players [[CITATION: http://fivethirtyeight.com/features/how-were-predicting-nba-player-career/]].  
...
and wind power (\citealp{jeon2012KCDEWindPower}) among others.  Methods similar
to those we explore in this article can also be formulated in the Bayesian
framework.  One example along these lines is
\citet{zhou2015DirichletProcessCopulaAmphibianDiseaseArrival}, who model the
...
through KCDE.  Methods combining nonparametric
estimates of marginal densities with copulas have been considered for other
applications before.  For example,
\citet{patton2012reviewCopulaEconomicTimeSeries} is a recent review of copula
methods for economic and financial time series including several
articles that have used nonparametric estimates for the marginal distributions.  Our
approach differs from these in the details of how the nonparametric marginal
distribution estimates are obtained.
...
For each $y^h_t$, $K^{Y | \bX}$ is a density function that contributes
...
contribution is largest near $z_{t + h}$.
...
The distributions that we simulate from are discretized bivariate normal
distributions.  To define this
...
the testing set in the dengue data was set by the competition
administrators, and we have adopted that convention of four years for use with
the influenza data as well.  All predictions are made as though in real time assuming that
...
There are three prediction targets for each data set, based closely on the
...
is a challenging task.  We have presented a semiparametric approach to doing
...
those methods is beyond the scope of this article; see
\citet{brown2016IDPredictionReview} for a review.  KCDE is distinguished from these approaches in that it
...
{\tt C}.  All source code as well as the data we used in the applications
are available in {\tt R} packages hosted on GitHub
...
available.  This work was funded in part by grants *******.

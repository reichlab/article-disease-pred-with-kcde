...
&\quad c^{H_{t^*}}\{f^{1}(z_{t^* + 1} | t^*, z_{t^* - l_1}, \ldots, z_{t^* - l_M}; \btheta^1), \ldots, f^{H_{t^*}}(z_{t^* + H_{t^*}} | t^*, z_{t^* - l_1}, \ldots, z_{t^* - l_M}; \btheta^{H_{t^*}}) ; \bxi^{H_{t^*}}\}. \label{eqn:ModelKCDECopula}
\end{align}
...
We followed the model selection and estimation procedures outlined in Held and
Paul \cite{Held2012HHHSeasonality} (see Supplement for details).  Because
...
variable selection with shrinkage.  Hall \etal
\cite{hall2004crossvalidationKCDE} show that when cross-validation is used to select the bandwidth parameters in

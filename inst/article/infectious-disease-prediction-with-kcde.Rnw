...
\title{Infectious disease prediction with kernel conditional density estimation}
...
allows for a full
parameterization of the bandwidth matrix with discrete variables. We demonstrate
via simulation that using a fully parameterized bandwidth matrix can
improve conditional density estimates. 
In applications to predicting dengue fever and influenza, our method outperforms a seasonal
...
fever.}
{copula, dengue fever, infectious disease, influenza, kernel
...
(\citealp{hatchett2007interventionsIntensity1918flu}).  Accurate 
predictions can help target resource allocation more effectively.
...
(\citealp{PandemicPredictionandForecastingScienceandTechnologyInteragencyWorkingGroup2015Announcement,
...
incidence.  In addition to the novel application of these methods to predicting
...
relationship between conditioning and outcome variables. Applications range from similar infectious disease settings where nearest neighbors regression has been used to make point predictions for incidence of
measles (\citealp{sugihara1990nonlinearForecasting}) and
influenza (\citealp{viboud2003predictionInfluenzaMoA}) to sports analytics
where a version of nearest neighbors regression predicts the career trajectories
of current NBA players (\citealp{SilverCARMELOPrediction}).
...
KCDE ({e.g.}, \citealp{HastieTibshiraniESL} discuss the connection between
...
KCDE has not previously been
applied to obtain predictive distributions for infectious disease incidence, but
it has been successfully used for prediction in other settings such as survival
time of lung cancer patients (\citealp{hall2004crossvalidationKCDE}), female
labor force participation (\citealp{hall2004crossvalidationKCDE}), bond yields
and value at risk in financial markets (\citealp{fan2004crossvalidationKCDE}),
and wind power (\citealp{jeon2012KCDEWindPower}), among others.  Similar methods
...
(\citealp{aitchison1976multivariateBinaryKernel,
...
in many cases (\citealp{duong2005crossvalidationBandwidthMultivariateKDE}).  We
...
scale well with the dimension of the vector whose distribution is being estimated (\citealp{HastieTibshiraniESL}).  This
...
(\citealp{patton2012reviewCopulaEconomicTimeSeries}) before.
...
literature on Gaussian Processes (\citealp{mackay1998introductionGP}), and is
defined by
\begin{equation}
...
distribution (\citealp{gneiting2007strictlyProperScoringRules}).
However, its use as an optimization criterion has been criticised for being
sensitive to outliers (\citealp{gneiting2007strictlyProperScoringRules}).  In
...
(\citealp{schuster1981nonconsistencyLCVforKDE,
...
We use copulas (\citealp{nelsen2007introductionCopulas}) to tie the marginal
...
(\citealp{HofertRCopulaPackage}).  The copula function is given by
\begin{equation}
...
(\citealp{joe2005asymptoticEfficiencyTwoStageCopula}).
...
integrated squared error (\citealp{kanazawa1993hellingerDistanceKDE}). For each
...
We use a seasonal autoregressive integrated moving average (SARIMA) model as a
baseline to compare our methods to.  In fitting this model, we first
...
%(\citealp{hyndmanRForecastPackage}); this function uses a stepwise procedure to
...
much more concentrated around its center.  Both methods struggle at larger
...
noticeable distinction between the predictions given by the two methods.
...
to improved predictions; this can be seen directly in Figure 4 of the
...
For both of these prediction targets, there is no consistent pattern of KCDE
either outperforming or underperforming relative to SARIMA.
...
\citealp{fan2004crossvalidationKCDE}) or the continuous ranked probability
score (\citealp{jeon2012KCDEWindPower}).
...
from these approaches in that it makes minimal assumptions about the data generating process.  This can be
...
The estimation methods were implemented in {\tt R} (\citealp{RCoreLanguage}) and
{\tt C}.  All source code and data are available in {\tt R} packages hosted on GitHub
(\citealp{ReichLabGitHubDiseasePredWithKCDEPackage}).
...
The authors thank Michael Johansson for helpful commentary on a draft of the
paper and the competition administrators for making disease incidence data
...
\begin{figure}
<<FluDataResultsAggregatedBoxplots, echo = FALSE, fig.height = 7.8, dev = "postscript">>=
models_used <- unique(ili_prediction_results$full_model_descriptor[
    !ili_prediction_results$differencing & !(ili_prediction_results$max_seasonal_lag == 1)])

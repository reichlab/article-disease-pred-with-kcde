...
(SARIMA) model and a previously published extension to the generalized linear
model framework developed for infectious disease incidence known as HHH4.
...
comparisons (such as the Diebold-Mariano test[cite]). For example, the standard
...
variance are different in different seasons, and are likely a function of
variables such as the timing and severity of the season peak. Additionally, due
to the limited amount of real data available, we have only a small number of
testing seasons to evaluate; this makes fitting a more flexible linear mixed
effects model with a realistic variance structure difficult. Therefore, many of
the conclusions about relative model performance are based on exploratory and
graphical summaries of performance metrics. Despite the lack of a formal
statistical test, we believe that the graphical summaries in Figures 4 and 6
show clear patterns of model performance, and in particular highlight the
...
ili_plot <- ggplot() +
    ...
    scale_x_date() +
    xlab("Time") +
    ylab("Weighted Proportion of Doctor Visits\nwith Influenza-like Illness (wILI)\n") +
...
for(row_num in 1:6) {
  ...
  cat(paste0(
    ...
    " & ",
    min_open_string,
    format(round(temp[row_num, "min"], 3), nsmall = 3),
    min_close_string,
#    " & ",
#    format(round(temp[row_num, "Q1"], 3), nsmall = 3),
#    " & ",
#    format(round(temp[row_num, "median"], 3), nsmall = 3),
    " & ",
    max_open_string,
    format(round(temp[row_num, "mean"], 3), nsmall = 3),
    max_close_string,
#    " & ",
#    format(round(temp[row_num, "Q3"], 3), nsmall = 3),
#    " & ",
#    format(round(temp[row_num, "max"], 3), nsmall = 3),
    "\\\\\n"
  ))
}
...
#     " & ",
#     min_open_string,
#     format(round(temp_nadir[row_num, "min"], 3), nsmall = 3),
#     min_close_string,
#     " & ",
#     format(round(temp_nadir[row_num, "Q1"], 3), nsmall = 3),
#     " & ",
#     format(round(temp_nadir[row_num, "median"], 3), nsmall = 3),
#     " & ",
#     max_open_string,
#     format(round(temp_nadir[row_num, "mean"], 3), nsmall = 3),
#     max_close_string,
#     " & ",
#     format(round(temp_nadir[row_num, "Q3"], 3), nsmall = 3),
#     " & ",
#     format(round(temp_nadir[row_num, "max"], 3), nsmall = 3),
#     "\\\\\n"
#   ))
# }
...
for(row_num in 1:6) {
  ...
  cat(paste0(
    ...
    " & ",
    min_open_string,
    format(round(temp_peak[row_num, "min"], 3), nsmall = 3),
    min_close_string,
#    " & ",
#    format(round(temp_peak[row_num, "Q1"], 3), nsmall = 3),
#    " & ",
#    format(round(temp_peak[row_num, "median"], 3), nsmall = 3),
    " & ",
    max_open_string,
    format(round(temp_peak[row_num, "mean"], 3), nsmall = 3),
    max_close_string,
#    " & ",
#    format(round(temp_peak[row_num, "Q3"], 3), nsmall = 3),
#    " & ",
#    format(round(temp_peak[row_num, "max"], 3), nsmall = 3),
    "\\\\\n"
  ))
}
...
ili_coverage <- left_join(ili_ribbons_df %>% mutate(time = as.Date(prediction_time)),
    ili_national,
    by = "time") %>%
...
dengue_coverage <- left_join(dengue_ribbons_df %>% mutate(time = as.Date(prediction_time)),
  dengue_sj,
  by = "time") %>%
...
#      coverage_for_table[row_num, 3], " & ",
      ifelse(best_in_group == row_num,
        "\\textbf{",
        ""),
      format(round(coverage_for_table[row_num, 4], 3), nsmall = 3),
      ifelse(best_in_group == row_num,
        "}",
        ""),
      " & ",
      ifelse(best_in_second_group == second_row_num,
        "\\textbf{",
        ""),
      format(round(coverage_for_table[second_row_num, 4], 3), nsmall = 3),
      ifelse(best_in_second_group == second_row_num,
        "}",
        ""),
      "\\\\\n"
...
for(row_num in 1:7) {
  ...
  cat(paste0(
    ...
    " & ",
    min_open_string,
    format(round(temp_before_peak[row_num, "min"], 3), nsmall = 3),
    min_close_string,
#    " & ",
#    format(round(temp_before_peak[row_num, "Q1"], 3), nsmall = 3),
#    " & ",
#    format(round(temp_before_peak[row_num, "median"], 3), nsmall = 3),
    " & ",
    max_open_string,
    format(round(temp_before_peak[row_num, "mean"], 3), nsmall = 3),
    max_close_string,
#    " & ",
#    format(round(temp_before_peak[row_num, "Q3"], 3), nsmall = 3),
#    " & ",
#    format(round(temp_before_peak[row_num, "max"], 3), nsmall = 3),
    "\\\\\n"
  ))
}
cat("\\midrule\n");
...
p_boxplots <- ggplot() +
    ...
    geom_point(aes(x = reduced_model_descriptor, y = mean_log_score_difference),
      colour = "red",
      shape = "-",
      size = 8,
      data = rbind.fill(
      ...
      group_by_(.dots = c("reduced_model_descriptor", "data_set_and_season", "prediction_target")) %>%
      summarize(mean_log_score_difference = mean(log_score_difference, na.rm = TRUE))
    ) +
...
## code to get counts of number of pre-peak weeks in each season:
#rbind.fill(
#    rbind.fill(
#        ili_peak_timing_log_score_diffs_from_equal_long[ili_peak_timing_log_score_diffs_from_equal_long$before_peak_week, ],
#        dengue_peak_timing_log_score_diffs_from_equal_long[dengue_peak_timing_log_score_diffs_from_equal_long$before_peak_week, ]
...
#) %>%
#group_by_(.dots = c("reduced_model_descriptor", "data_set_and_season", "prediction_target")) %>%
#summarize(num_obs = length(log_score_difference)) %>%
#arrange(data_set_and_season) %>%
#as.data.frame()
...
#            facet_wrap(~ data_set_and_season, ncol = 1) +
            ...
            ylab("") +
            theme_bw() +
            suppressWarnings(theme(panel.margin = unit(0, "mm"),

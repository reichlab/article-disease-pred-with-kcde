...
\begin{enumerate}
  ...
  \item Incidence in the peak week of the current season.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item Binned incidence in the peak week of the current season:
    ...
    \end{align}
\end{enumerate}
In predicting binned incidence in the peak week, we are following the precedent set in the
competitions run by the CDC.
...
Across all three prediction targets, an important benefit of the proposed method is that
it consistently places non-negligible probability on the eventually observed outcome.
This is in contrast to the baseline models we considered, which occasionally missed quite badly.
These improvements in the reliability of
predictions are valuable to public health officials planning interventions several weeks or
months before the peak of the disease season.
However, since year-to-year
...
We have introduced the use of a periodic kernel component that led to substantial improvements in the predictive distributions for incidence in individual weeks. Periodic kernels have not otherwise been used in the KCDE literature despite their importance for prediction in seasonal systems. This advance improves the applicability of KCDE to infectious disease prediction in general and we demonstrate how it leads to improved performance for CDC's chosen prediction targets.

We also introduced application of KCDE with a fully parameterized bandwidth matrix to discrete data. Much infectious disease case data is discrete and small discrete counts can be indicative of transmission dynamics driven by stochasticity[ cite a stochastic SIR (?) ]. Handling this important case directly makes our method widely applicable to infectious disease data, particularly when combined with the periodic kernel.
...
this article could be applied to make predictions with multiple time series, although we have not pursued that line in this work.
For example, one possible approach to this would be to fit a separate predictive model for
each time series, using the other time series as covariates that are conditioned on.  If a joint distribution of these time series were required, we could use the copula to estimate joint dependence structure across all of the time series.  Another option would be to use KCDE to directly estimate the joint distribution of a random vector of incidence in future time points.

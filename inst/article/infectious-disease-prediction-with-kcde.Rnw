...
variables.  We demonstrate via simulation that using a fully parameterized
bandwidth matrix can be beneficial for estimating discrete conditional densities.
We applied the method to predicting dengue fever and influenza, and compared to
...
incidence known as HHH4.  KCDE outperformed the baseline methods for
predictions of dengue incidence in individual weeks.  KCDE also offered more
consistent performance than the baseline models for predictions of
incidence in the peak week, and was comparable to the baseline models on the
...
\flushleft{copula, dengue fever, infectious disease, influenza, kernel
...
incidence in a single week using KCDE as outlined in
Subsection~\ref{subsec:Methods:KCDE}: predicting incidence at time $t + h$ given
...
(Supplemental Figure ***).  In this simulation study we demonstrate that
...
The results for predictions at times of high incidence are visible in
...
the dynamics of dengue incidence accurately.  For example,
\cite{Held2012HHHSeasonality} discuss the fact that the seasonal structure in
the HHH4 model does not explicitly allow for different onset times and amplitudes in different seasons.
